                ...
                foreach($arr as $x){
                    DB::unprepared(
                        DB::raw('
                            ...
                            UPDATE '.$table_name_cols.' t, activities a SET t.m'.$ref.'_com=(SELECT sum(task_hour) FROM `activities` where month ='.$month['month'].' and year ='.$month['year'].' and user_id = '.$x.'), t.m'.$ref.'_from_otl=0,t.m'.$ref.'_id=0 where t.user_id='.$x.';
                        ')
                    );
                }
            }
